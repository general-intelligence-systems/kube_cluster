...
module Kube
  module Cluster
    class Manifest < Kube::Schema::Manifest
      ...
      #
      # Middleware transforms a single resource as it flows out of the
      # manifest enumerator. Subclasses override +call+ to inspect
      # and/or transform the resource.
      #
      #   class MyMiddleware < Kube::Cluster::Manifest::Middleware
      #     def call(resource)
      #       h = resource.to_h
      #       h[:metadata][:labels] ||= {}
      ...
      class Middleware
        ...
        # Override in subclasses. Receives a single Resource, returns a
        # single Resource (possibly rebuilt with transformed data).
        def call(resource)
          resource
        end
