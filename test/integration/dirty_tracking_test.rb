...
class TestConfigMap < Kube::Cluster::Resource
  ...
  def name          = to_h[:name]&.to_s
  def ns_flags      = to_h[:namespace] ? ["--namespace", to_h[:namespace].to_s] : []
...
class DirtyTrackingIntegrationTest < Minitest::Test
  ...
  def test_full_apply_mutate_patch_lifecycle
    resource, ctl = build_resource(name: "app-config", namespace: "production", data: { key: "original" })

    # Stub the reload after apply — server echoes back what we sent
    ctl.stub_response("get", server_state(
      name: "app-config", namespace: "production", data: { key: "original" },
    ...
    # Mutate
    resource.instance_variable_get(:@data).data.key = "updated"
    ...
    ctl.stub_response("get", server_state(
      name: "app-config", namespace: "production", data: { key: "updated" },
  ...
  def test_patch_returns_false_when_clean
    resource, ctl = build_resource(name: "app-config", namespace: "default", data: { key: "value" })

    ctl.stub_response("get", server_state(
      name: "app-config", namespace: "default", data: { key: "value" }
    ))
  ...
  def test_patch_sends_only_changed_fields
    resource, ctl = build_resource(
      name: "my-config",
      namespace: "staging",
      data: { db_host: "old-db.internal", db_port: "5432", cache_ttl: "300" }
    )

    # Mutate one field
    resource.instance_variable_get(:@data).data.db_host = "new-db.internal"

    ctl.stub_response("get", server_state(
      name: "my-config", namespace: "staging",
      data: { db_host: "new-db.internal", db_port: "5432", cache_ttl: "300" }
    ))
    ...
    # The payload should contain the data subtree but NOT name/namespace
    assert payload.key?("data"), "patch payload should include changed subtree"
    refute payload.key?("name"), "patch payload should not include unchanged top-level keys"
    refute payload.key?("namespace"), "patch payload should not include unchanged top-level keys"
  end
  ...
  def test_reload_resets_dirty_state
    resource, ctl = build_resource(name: "my-config", namespace: "default", data: { key: "v1" })

    # Local mutation
    resource.instance_variable_get(:@data).data.key = "local-change"
    assert resource.changed?

    # Server still has original
    ctl.stub_response("get", server_state(
      name: "my-config", namespace: "default", data: { key: "v1" }
    ))
    ...
    refute resource.changed?
    assert_equal "v1", resource.to_h[:data][:key]
  end
  ...
  def test_reload_picks_up_server_side_changes
    ...
    ctl.stub_response("get", server_state(
      name: "my-config", namespace: "default", data: { key: "server-updated" },
    ...
    refute resource.changed?
    assert_equal "server-updated", resource.to_h[:data][:key]
  end
  ...
  def test_apply_snapshots_server_response
    resource, ctl = build_resource(name: "my-config", data: { key: "v1" })

    # Server adds metadata on apply
    ctl.stub_response("get", server_state(
      name: "my-config", data: { key: "v1" },
      metadata: { resourceVersion: "1", uid: "abc-123" }
    ))
    ...
    resource.instance_variable_get(:@data).data.key = "v2"
    changes = resource.changes

    # changes[:data] is [old_hash, new_hash]
    old_data, new_data = changes[:data]
    assert_equal "v1", old_data[:key]
    assert_equal "v2", new_data[:key]
  ...
  def test_patch_raises_on_unpersisted_resource
    resource, _ctl = build_resource(data: { key: "value" })
  ...
  def test_nested_mutation_produces_nested_patch
    resource, ctl = build_resource(
      name: "my-config",
      namespace: "default",
      metadata: { labels: { app: "web", tier: "frontend" }, namespace: "default" }
    )
    ...
    refute patch.key?(:name), "unchanged top-level key should not appear in patch"
  end
  ...
  def test_multiple_mutations_coalesce_in_single_patch
    ...
    data = resource.instance_variable_get(:@data).data
    data.host = "db-2"
    data.port = "5433"
    data.pool = "10"
  ...
  def test_changes_applied_resets_baseline_without_server_roundtrip
    resource, _ctl = build_resource(name: "my-config", data: { key: "v1" })

    resource.instance_variable_get(:@data).data.key = "v2"
    assert resource.changed?
    assert_equal([:data], resource.changed)
    ...
    resource.instance_variable_get(:@data).data.key = "v3"
    assert resource.changed?

    changes = resource.changes
    # Old value should be v2 (the accepted baseline), not v1
    assert_equal "v2", changes[:data].is_a?(Hash) ? changes[:data][:key]&.first : nil,
      "baseline should be v2 after changes_applied" if changes[:data].is_a?(Hash)
    assert_equal({ data: [{ key: "v2" }, { key: "v3" }] }, changes) if changes[:data].is_a?(Array)
  end
  ...
  def test_attr_changed_through_apply_mutate_patch_cycle
    ...
    refute resource.data_changed?, "data should not be changed after apply"
    refute resource.name_changed?, "name should not be changed after apply"

    resource.instance_variable_get(:@data).data.key = "v2"

    assert resource.data_changed?, "data should be changed after mutation"
    refute resource.name_changed?, "name should still not be changed"

    ctl.stub_response("get", server_state(
      name: "my-config", namespace: "default", data: { key: "v2" }
    ))

    resource.patch

    refute resource.data_changed?, "data should not be changed after patch"
  end

  def test_respond_to_for_dynamic_changed_predicates
    resource, _ctl = build_resource(name: "test")

    assert resource.respond_to?(:name_changed?)
    assert resource.respond_to?(:data_changed?)
  ...
  def test_reload_does_not_corrupt_previously_captured_changes
    ...
    ctl.stub_response("get", server_state(
      name: "my-config", namespace: "default", data: { key: "v3-from-server" }
    ))

    resource.reload

    # Previously captured hashes should be unaffected
    assert_equal "v2", extract_nested_value(changes_before, :data, :key, 1),
      "previously captured changes should not be corrupted by reload"
    assert_equal "v2", extract_nested_value(patch_before, :data, :key, 1),
  ...
  def test_snapshot_isolation_across_multiple_changes_applied
    resource, _ctl = build_resource(name: "test", data: { counter: "1" })
  ...
  # -------------------------------------------------------------------------
  # Edge case: resource with no initial data
  # -------------------------------------------------------------------------

  def test_empty_resource_tracks_all_additions
    resource, _ctl = build_resource(name: "empty-config")

    resource.instance_variable_get(:@data).data = BlackHoleStruct.new(key: "added")

    assert resource.changed?
    assert_includes resource.changed, :data
  end
  ...
  def test_patch_includes_namespace_flags
    resource, ctl = build_resource(name: "my-config", namespace: "kube-system", data: { key: "v1" })

    resource.instance_variable_get(:@data).data.key = "v2"

    ctl.stub_response("get", server_state(
      name: "my-config", namespace: "kube-system", data: { key: "v2" }
    ))
  ...
  def test_reload_includes_namespace_flags
    resource, ctl = build_resource(name: "my-config", namespace: "monitoring", data: { key: "v1" })

    ctl.stub_response("get", server_state(
      name: "my-config", namespace: "monitoring", data: { key: "v1" }
    ))
  ...
  def test_delete_issues_kubectl_delete
    resource, ctl = build_resource(name: "my-config", namespace: "default")
  ...
  private
    ...
    # changes[:data] could be [old_hash, new_hash] or a nested diff hash.
